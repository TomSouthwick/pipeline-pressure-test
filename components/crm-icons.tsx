/** Brand accent colours, used for the export chip's border + faint tint. */
export const CRM_BRAND: Record<"salesforce" | "hubspot", string> = {
  salesforce: "#00A1E0",
  hubspot: "#ff4800",
};

/** Salesforce cloud mark (brand blue). Path from Simple Icons. */
export function SalesforceIcon({ size = 15 }: { size?: number }) {
  return (
    <svg
      width={size}
      height={size}
...
/** HubSpot sprocket mark (brand coral). Path from Simple Icons. */
export function HubSpotIcon({ size = 15 }: { size?: number }) {
  return (
    <svg
      width={size}
      height={size}
